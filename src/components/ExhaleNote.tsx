...
import { renderIntoPrintRoot } from "./exhale-note/printer";
...
function makeFileTitle(kaiSig: string, pulse: string, stamp: string): string {
  const serialCore = (kaiSig ? kaiSig.slice(0, 12).toUpperCase() : "SIGIL").replace(
    /[^0-9A-Z]/g,
    "Φ"
  );
  const safe = (s: string) =>
...
/** Inject preview CSS once so the SVG scales on mobile */
...
/** Injects print CSS overrides so the print root actually renders */
function ensurePrintStylesInjected(): void {
  ...
  style.textContent = `
    ...
    @media print {
      ...
      /* IMPORTANT: show when aria-hidden="false" (matches JS below) */
...
/** rAF throttle for heavy preview work while in live mode */
...
async function computeValuationStamp(u: IntrinsicUnsigned): Promise<string> {
  ...
  return sha256HexCanon(`val-stamp:${JSON.stringify(minimal)}`);
}
...
const ExhaleNote: React.FC<NoteProps> = ({
  ...
  const [pulse, setPulse] = useState<number>(() =>
    getNowPulse ? getNowPulse() : kaiPulseNowBridge()
  );
  ...
  const [form, setForm] = useState<BanknoteInputs>({
    ...
    zk: undefined,
    sigilSvg: "",
    verifyUrl: typeof window !== "undefined" ? `${window.location.origin}/` : "/",
    ...(initial ?? {}),
  });

  /* Lock state (single render flow) */
  const [locked, setLocked] = useState<ExhaleNoteRenderPayload | null>(null);
  const lockedRef = useRef(false);
  const [isRendering, setIsRendering] = useState(false);

  /* Prevent preview loops after lock */
  const lockedPreviewOnceRef = useRef(false);
  const lastSafeBridgePayloadRef = useRef<Partial<BanknoteInputs> | null>(null);
  ...
  const liveUnsigned = useMemo<IntrinsicUnsigned>(() => {
    const { unsigned } = computeIntrinsicUnsigned(meta, nowFloor) as {
  ...
  useEffect(() => {
    setForm((prev) =>
      prev.valuationAlg ? prev : { ...prev, valuationAlg: liveAlgString }
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [liveAlgString]);
  ...
  const buildCurrentSVG = useCallback((): string => {
    ...
    const valuePhiStr = usingLocked
      ? fTiny((locked as ExhaleNoteRenderPayload).valuePhi)
      : fTiny(liveValuePhi);

    const premiumPhiStr = usingLocked
      ? form.premiumPhi || fTiny(livePremium)
      : fTiny(livePremium);

    const lockedPulseStr = usingLocked
      ? String((locked as ExhaleNoteRenderPayload).lockedPulse)
      : "";

    const valuationStampStr = usingLocked
      ? form.valuationStamp || (locked as ExhaleNoteRenderPayload).seal.stamp
      : "";
  ...
  // Live: throttle; Locked: render exactly once
  useEffect(() => {
    const host = previewHostRef.current;
    if (!host) return;

    if (locked) {
      if (lockedPreviewOnceRef.current) return;
      lockedPreviewOnceRef.current = true;
      try {
        renderPreview(host, buildCurrentSVG());
      } catch (e) {
        // eslint-disable-next-line no-console
    ...
    lockedPreviewOnceRef.current = false;
    renderPreviewThrottled();
  }, [buildCurrentSVG, renderPreviewThrottled, locked]);
  ...
  const handleRenderLock = useCallback(async () => {
    ...
    try {
      ...
      const sealedBase: ValueSeal = materializeStampedSeal(
        lockedUnsigned as unknown as MaybeUnsignedSeal
      );
      ...
      setForm((prev) => ({
        ...
        valuationStamp,
        premiumPhi:
          lockedUnsigned.premium !== undefined ? fTiny(lockedUnsigned.premium) : prev.premiumPhi,
        valuationAlg:
          prev.valuationAlg || `${lockedUnsigned.algorithm} • ${lockedUnsigned.policyChecksum}`,
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!lockedRef.current) renderPreviewThrottled();
      } catch (e) {
    ...
    const onEvt: EventListener = (evt: Event) => {
      try {
        ...
        if (lockedRef.current) {
          const unsafeKeys = new Set<keyof BanknoteInputs>([
            "valuePhi","premiumPhi","computedPulse","nowPulse","valuationAlg","valuationStamp",
          ]);
          const safe = Object.fromEntries(
            Object.entries(detail).filter(([k]) => !unsafeKeys.has(k as keyof BanknoteInputs))
          ) as Partial<BanknoteInputs>;

          const lastSafe = lastSafeBridgePayloadRef.current || {};
          if (shallowEqual(lastSafe as Partial<BanknoteInputs>, safe)) return;
          lastSafeBridgePayloadRef.current = safe;
    ...
    window.addEventListener("kk:note-data", onEvt, { passive: true } as AddEventListenerOptions);
    return () => {
      active = false;
  ...
  const onPrint = useCallback(async () => {
    ...
    renderIntoPrintRoot(root, banknote, proofPages, String(locked.lockedPulse));
    ...
    <div data-kk-scope={uid} className={`kk-note ${className ?? ""}`}>
      ...
      <section className={`kk-hero ${locked ? "is-locked" : "is-live"}`}>
        <div className="kk-status">
          <span className={`kk-chip ${locked ? "chip-locked" : "chip-live"}`}>
            {locked ? "LOCKED" : "LIVE"}
          </span>
            ...
            {!locked ? (
              ...
              <div className="kk-locked-banner" role="status" aria-live="polite">
                ...
                <div className="kk-locked-sub">
                  Pulse {locked.lockedPulse} • Hash: {form.valuationStamp || "—"}
                </div>
              </div>
            )}
      ...
      <div className="kk-grid">
        <div className="kk-stack">
          ...
          <div className="kk-row">
            ...
            <input
              value={form.from}
              onChange={(e) => u("from")(e.target.value)}
              placeholder="Issuer"
              disabled={!!locked}
            />
          </div>
      ...
      <details className="kk-stack" style={{ marginTop: 8 }} open>
        <summary>
          <strong>Identity &amp; Valuation</strong>{" "}
          <span className="kk-hint">— appears on the bill + proof pages</span>
        </summary>
        <div className="kk-grid" style={{ marginTop: 8 }}>
          <div className="kk-stack">
            ...
            <div className="kk-row">
              ...
              <input value={locked ? form.valuationStamp || "—" : ""} readOnly />
            </div>
          </div>
          <div className="kk-stack">
            <div className="kk-row">
              ...
              <input
                value={locked ? String(locked.lockedPulse) : ""}
                readOnly
                placeholder="set on Render"
              />
            </div>
            <div className="kk-row">
            ...
            <div className="kk-row">
              ...
              <input
                value={form.kaiSignature}
                onChange={(e) => u("kaiSignature")(e.target.value)}
                disabled={!!locked}
              />
            </div>
            <div className="kk-row">
              ...
              <input
                value={form.userPhiKey}
                onChange={(e) => u("userPhiKey")(e.target.value)}
                disabled={!!locked}
              />
            </div>
            <div className="kk-row">
              ...
              <input
                value={form.sigmaCanon}
                onChange={(e) => u("sigmaCanon")(e.target.value)}
                disabled={!!locked}
              />
            </div>
            <div className="kk-row">
              ...
              <input
                value={form.shaHex}
                onChange={(e) => u("shaHex")(e.target.value)}
                disabled={!!locked}
              />
            </div>
            <div className="kk-row">
              ...
              <input
                value={form.phiDerived}
                onChange={(e) => u("phiDerived")(e.target.value)}
                disabled={!!locked}
              />
            </div>
          </div>
