...
const MS_PER_BEAT = (HARMONIC_DAY_PULSES * BREATH_SEC * 1000) / BEATS_PER_DAY; // ≈ 2_544_041.137 ms
...
export default function ValueHistoryModal({
  ...
  // IMPORTANT: the upstream hook keeps the same array identity while mutating its contents,
  // so we key our recomputations on length and the last point's t.
  const lastT = series.length ? series[series.length - 1].t : -1;
  const seriesVersion = `${series.length}:${lastT}`;
  ...
  }, [seriesVersion]); // ← recompute when new samples arrive

  // If the stream is still warming up, seed a single point from latestValue so the chart shows instantly.
  const seeded = useMemo<Point[]>(() => {
    if (kaiSorted.length > 0) return kaiSorted;
    if (!Number.isFinite(latestValue)) return [];
    return [{ t: msToAbsBeat(Date.now()), v: latestValue }];
  ...
  const { minT, maxT, minV, maxV } = useMemo(() => {
    ...
    return { minT: Math.min(...ts), maxT: Math.max(...ts), minV: vMin - pad, maxV: vMax + pad };
  }, [filtered]);
  ...
  useEffect(() => {
    ...
    if (hoverX != null && hoverY != null && hoverIdx != null && filtered[hoverIdx]) {
      ...
      ctx.setLineDash([4, 6]);
      ctx.beginPath(); ctx.moveTo(xx, padY); ctx.lineTo(xx, H - padY); ctx.stroke();
      ctx.setLineDash([]);

      ctx.fillStyle = "#fff";
      ctx.beginPath(); ctx.arc(xx, yy, 3, 0, Math.PI * 2); ctx.fill();

      const tip = `Beat ${p.t.toFixed(6)}  •  ${p.v.toLocaleString(undefined, { maximumFractionDigits: 6 })} Φ`;
      const pad = 8;
      ...
      const r = 8, w = tw + pad * 2, h = 24, x0 = bx, y0 = by;
  ...
  useEffect(() => {
    ...
    const onMove = (ev: PointerEvent) => {
      ...
      if (filtered.length) {
        ...
        const x01 = Math.min(1, Math.max(0, (cx / DPR - minX) / Math.max(1, maxX - minX)));
        ...
        const pick =
          Math.abs(filtered[lo].t - tGuess) < Math.abs(filtered[hi].t - tGuess) ? lo : hi;
        setHoverIdx(pick);
      }
    };

    const onLeave = () => { setHoverX(null); setHoverY(null); setHoverIdx(null); };
  ...
  return (
    <div className="valuehist-overlay" role="dialog" aria-modal="true" aria-label="Value History">
      <button className="valuehist-exit" onClick={onClose} title="Close">✕</button>
      ...
      <div className="valuehist-stage" style={{ alignItems: "stretch" }}>
        <div className="valuehist-panel">
          ...
          <div ref={wrapRef} className="valuehist-frame">
            {filtered.length === 0 && (
              <div className="valuehist-empty">Waiting for live samples…</div>
            )}
