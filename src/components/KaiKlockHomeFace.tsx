...
const KaiKlockHomeFace: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => { if (e.key === "Escape") setShowEternal(false); };
  ...
  const rotationOverride = useMemo(() => {
    ...
    const currentBeat = Math.floor(((d?.kaiPulseToday ?? 0) % HARMONIC_DAY_PULSES) / beatPulseCount);
    return ((currentBeat + 0.5) / 36) * 360;
  }, [d?.rotationOverride, d?.kaiPulseToday]);

  return (
    <>
      {/* ⬇️ NEW wrappers let the dial grow to the largest possible square */}
      <div
      ...
      {showEternal && portalTarget &&
        createPortal(
          <div
            ...
            ref={overlayRef}
            tabIndex={-1}
            onClick={(e) => { if (e.target === overlayRef.current) setShowEternal(false); }}
          >
            <button
              type="button"
            ...
            <div className="eternal-modal-card" onClick={(e) => e.stopPropagation()}>
              <EternalKlock />
            </div>
          </div>,
          portalTarget
