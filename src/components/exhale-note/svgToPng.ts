...
export function errorMessage(err: unknown): string {
  ...
  function cssLengthToPx(input: string): number | null {
    ...
    switch (unit) {
      case "px": return val;
      case "pt": return val * (PX_PER_IN / 72);  // 1pt = 1/72 in
      case "pc": return val * 16;                // 1pc = 12pt = 16px
      case "in": return val * PX_PER_IN;
      case "cm": return val * (PX_PER_IN / 2.54);
      case "mm": return val * (PX_PER_IN / 25.4);
      default: return null;
    }
  }
  ...
  function hasCloseMethod(
    b: ImageBitmap | null
  ): b is ImageBitmap & { close: () => void } {
  ...
  export async function svgStringToPngBlob(
    ...
    const opts: SvgToPngOptions =
      typeof outWidthOrOptions === "number" ? { outWidth: outWidthOrOptions } : outWidthOrOptions;
    ...
    if (targetW > MAX_SIDE || targetH > MAX_SIDE) {
      ...
      console.warn(
        `[svgToPng] Requested size ${targetW}x${targetH} exceeded limits. Clamped.`
      );
    ...
    try {
      ...
      if (background) {
        // save/restore are present in both contexts per spec
